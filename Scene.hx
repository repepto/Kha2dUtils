...
class Scene {
	...
	private var width: Int = 1200;
	private var height: Int = 780;
	...
	var particles:Array<ParticleSystem>;
	var movieClips:Array<MovieClip>;
	...
	public function addParticleSystem(ps:ParticleSystem)
	{
		if (particles == null) particles = new Array();
	...
	public function addGroup(g:SpriteGroup)
	{
		g.isAddedToScene = true;
		...
		for (s in g.sprites)
		{
			if (!s.isAddedToScene) Scene.the.addHero(s);
		}
	}
	
	public function removeGroup(g:SpriteGroup)
	{
		for (s in g.sprites)
		{
	...
	public function addMovieClip(m:MovieClip)
	{
		if (movieClips == null) movieClips = new Array();
		movieClips.push(m);
		for (f in m.frames)
		{
			the.addHero(f);
	...
	public function removeMovieClip(m:MovieClip)
	{
		movieClips.remove(m);
	}
	...
	public function update(): Void {
		...
		if(groups != null) for (g in groups)
		{
			g.update();
		}
		...
		if(particles != null) for (ps in particles)
		{
			ps.update();
		}
		
		if(movieClips != null) for (mc in movieClips)
		{
			mc.update();
		}
	}
	...
	public function render(g: Graphics) {
		...
		for (z in 0...10) {
			...
			while (i < sprites.length) {
				var ox = 0;
				if (sprites[i].parent != null) ox = sprites[i].parent.x;
				if (sprites[i].x + ox + sprites[i].width > screenOffsetX) break;
				++i;
			}
			while (i < sprites.length) {
				var ox = 0;
				if (sprites[i].parent != null) ox = sprites[i].parent.x;
				if (sprites[i].x + ox > screenOffsetX + width) break;
